...
async function mainThreadHandler(
  ...
  for (let page = 1; page <= pages; page++) {
    link = `http://mbrlkbtq5jonaqkurjwmxftytyn2ethqvbxfu4rgjbkkknndqwae6byd.onion/index.php?page=${String(
      page,
    )}`;
    items.push({
...
async function threadHandler(
  ...
  elements.forEach((el) => {
    const id = String(
      $(el).attr('onclick').trim().replace("viewtopic('", '')
        .replace("')", ''),
    );
    const link = `http://mbrlkbtq5jonaqkurjwmxftytyn2ethqvbxfu4rgjbkkknndqwae6byd.onion/topic.php?id=${String(
      id,
    )}`;
    const title = $(el)
      .text()
