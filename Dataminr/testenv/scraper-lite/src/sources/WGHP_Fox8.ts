...
const baseURL = 'https://myfox8.com/';
async function threadHandler(
  ...
  elements.forEach((el) => {
    ...
    if (moment(articlePublishedDate, 'YYYY-MM-DD').isSame(moment(), 'day')) {
      ...
      const headline = $(el).find('h2 a, h3 a').text().replace(/\n+/g, ' ')
        .replace(/\t+/g, ' ')
        .trim();
      threads.push({
        link: href,
...
export const parser = new LiteParser('WGHP Fox 8', baseURL, [
  {
    selector: ['#standard-layout article.article-list__article, #primary section.article-list article.article-list__article'],
